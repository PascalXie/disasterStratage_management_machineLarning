...
void GAGeneticAlgorithm::SetGeneSize(int GeneSize)
{
	...
	{
		cout<<"An warning occured in GAGeneticAlgorithm::SetGeneSize"<<endl;
		cout<<"GeneSize_ is larger than the Robot size the deploying evenly has!"<<endl;
		cout<<"GeneSize_ is set that equal to the Robot size the deploying evenly has!"<<endl;

		GeneSize_=toolRobotDeploymentEvenly_->robotCoor_x_2_.size();
		IsGeneSizeSet_ = true;
	}
}
