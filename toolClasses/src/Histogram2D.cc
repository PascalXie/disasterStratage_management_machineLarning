...
int Histogram2D::GetBinIDX(double x)
{
	...
	// range check
	if(x<minx_||x>=maxx_) return binxID;
...
{
	if(binIDx<0||binIDx>=binSizex_||binIDy<0||binIDy>=binSizey_)
	{
		//cout<<"An error happend in Histogram2D::GetBinCentor2D"<<endl;
		return false;
	}
...
{
	if(binIDx>=binSizex_||binIDy>=binSizey_||binIDx<0||binIDy<0)
	{
